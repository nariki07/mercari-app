...
$(function() {
	...
	$("#btnAdd").val("add").css({ "margin-bottom": "25px", "margin-top": "10px" }).on('click', function() {
		...
		let input_condition = '<div class="form-group" id="dynamicForm5' + n + '"><label for="condition" class="col-sm-2 control-label">condition</label><div class="col-sm-8"><label for="condition1" class="radio-inline"> <input type="radio" name="insertItemFormList[' + n + '].conditionId" id="condition1" value="1" th:field="*{insertItemFormList[' + n + '].conditionId}" checked /> 1</label> <label for="condition2" class="radio-inline"> <input type="radio" name="insertItemFormList[' + n + '].conditionId" id="condition2" value="2" th:field="*{insertItemFormList[' + n + '].conditionId}" /> 2</label> <label for="condition3" class="radio-inline"> <input type="radio" name="insertItemFormList[' + n + '].conditionId" id="condition3" value="3" th:field="*{insertItemFormList[' + n + '].conditionId}" /> 3 </label></div></div><div class="form-group"><label for="category" class="col-sm-2 control-label"></label><div class="col-sm-8"><div th:errors="*{insertItemFormList[' + n + '].conditionId}" style="color: red" class="error-messages"></div></div></div>'
		let input_description = '<div class="form-group" id="dynamicForm6' + n + '"><label for="description" class="col-sm-2 control-label">description</label><div class="col-sm-8"><textarea name="insertItemFormList[' + n + '].description" id="description" class="form-control"rows="5" th:field="*{insertItemFormList[' + n + '].description}"></textarea><div th:errors="*{insertItemFormList[' + n + '].description}" style="color: red" class="error-messages"></div></div></div></div>'
